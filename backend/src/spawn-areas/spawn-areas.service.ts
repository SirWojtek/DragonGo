...
export class SpawnAreasService {
  ...
  private async fetchFragment(region: Rect): Promise<SpawnAreaEnity[]> {
    ...
    // TODO: check if region < MAP_FRAGMENT_SIZE
    if (!center) {
      // TODO: log this
      return [];
    }

    const [places, mapFragment] = await Promise.all([
      this.googleMapsService.getPlaces(
        { lat: center.latitude, lng: center.longitude },
        MAP_FRAGMENT_SIZE,
      ),
      this.createMapFragment({ lat: center.latitude, lng: center.longitude }),
    ]);
  ...
  private createMapFragment(center: LatLng): Promise<MapFragmentEntity> {
    const northeast = computeDestinationPoint(center, MAP_FRAGMENT_SIZE, 45);
    const southwest = computeDestinationPoint(center, MAP_FRAGMENT_SIZE, 225);
